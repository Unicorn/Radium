...
impl CodeChanges {
    ...
    pub fn from_git_since(workspace_root: &Path, since_timestamp: u64) -> Result<Self> {
        ...
        let since_date = chrono::DateTime::from_timestamp(since_timestamp as i64, 0).map_or_else(
        ...
        let first_commit = first_commit_str.lines().next().unwrap_or("HEAD").to_string();
