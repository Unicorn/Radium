...
mod ignore_tests {
    ...
    fn test_should_ignore_path() {
        assert!(should_ignore_path(std::path::Path::new("target/file.rs")));
        assert!(should_ignore_path(std::path::Path::new("src/node_modules/file.js")));
        assert!(!should_ignore_path(std::path::Path::new("src/file.rs")));
    }
}
