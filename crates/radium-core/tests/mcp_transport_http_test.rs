...
use radium_core::mcp::transport::HttpTransport;
use radium_core::mcp::McpTransport;
use std::time::Duration;

#[tokio::test]
async fn test_http_transport_connection_lifecycle() {
    let mut transport = HttpTransport::new("http://localhost:9999".to_string());
...
#[tokio::test]
async fn test_http_transport_double_connect() {
    let mut transport = HttpTransport::new("http://localhost:9999".to_string());
...
#[tokio::test]
async fn test_http_transport_send_receive() {
    let mut transport = HttpTransport::new("http://localhost:9999".to_string());
...
#[tokio::test]
async fn test_http_transport_with_auth() {
    let mut transport = HttpTransport::new_with_auth(
...
#[tokio::test]
async fn test_http_transport_connection_timeout() {
    let mut transport = HttpTransport::new("http://192.0.2.0:9999".to_string());
...
#[tokio::test]
async fn test_http_transport_send_with_auth() {
    let mut transport = HttpTransport::new_with_auth(
        "http://localhost:9999".to_string(),
        Some("Bearer test-token".to_string()),
    );
...
#[tokio::test]
async fn test_http_transport_multiple_operations() {
    let mut transport = HttpTransport::new("http://localhost:9999".to_string());
...
#[tokio::test]
async fn test_http_transport_error_status_handling() {
    // Test that non-2xx status codes are handled
    let mut transport = HttpTransport::new("http://localhost:9999/error".to_string());
    
    // Connect might succeed (if server returns 404, which we allow)
    if transport.connect().await.is_ok() {
        // Send should handle error responses
        let result = transport.send(b"test").await;
        // Should either succeed or return a clear error
        if let Err(e) = result {
            assert!(e.to_string().contains("Failed to send") || e.to_string().contains("error"));
        }
