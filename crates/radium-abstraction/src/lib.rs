...
///
/// Represents different types of items in a streaming response.
///
/// This enum allows distinguishing between thinking process tokens and final answer tokens,
/// enabling real-time display of the model's reasoning process.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum StreamItem {
    /// Token from the model's thinking/reasoning process.
    /// Displayed before the final answer to show how the model arrived at its conclusion.
    ThinkingToken(String),
    /// Token from the final answer.
    /// This is the actual response content that should be displayed normally.
    AnswerToken(String),
    /// Metadata update (e.g., usage information, finish reason).
    /// Contains key-value pairs of metadata that may be updated during streaming.
...
pub trait StreamingModel: Send + Sync {
    ...
    ///
    /// Returns an async stream that yields stream items as they're generated by the model.
    /// Each item in the stream is a `Result<StreamItem, ModelError>` where:
    /// - `Ok(StreamItem::ThinkingToken(token))` contains a token from the thinking process
    /// - `Ok(StreamItem::AnswerToken(token))` contains a token from the final answer
    /// - `Ok(StreamItem::Metadata(map))` contains metadata updates
    /// - `Err(error)` indicates an error during generation
    ///
    /// For non-thinking models, all tokens will be `AnswerToken`.
    ///
    /// # Arguments
    /// * `prompt` - The input prompt for text generation
    /// * `parameters` - Optional parameters to control generation (including reasoning_effort)
    ///
    /// # Returns
    /// A pinned, boxed stream of stream items. The stream must be `Send` to allow
